...
from PRISMA_SDS import PRIS_tools
...
def generate_PAN(directories, error_matrix):
    ...
    for img_name in images_names:
        ...
        GeoT, driver, Proje = PRIS_tools.get_GeoTransform(geo, panchro2, res)
...
def generate_HS(directories, error_matrix, save_RGB):
    ...
    for img_name in images_names:
        ...
        GeoT, driver, Proje = PRIS_tools.get_GeoTransform(geo, img3, res)  
...
def crop_images(directories):
    ...
    beach = directories['PanSharp_Square_Cropped'].split('/')[-3] 
        
    # Load the polygon
    
    inshape = os.path.join(directories['Crop_shp'], beach + '_SquarePol.shp')
    ...
    for hs_img in hs_images:
        ...
        date = hs_img.split('_')[3]
        
        inraster = os.path.join(directories['HS'], hs_img)
        
        # Define the image output name
        
        file_out = beach + '_' + date + '_HS_sub'+ '.tiff'
    ...
    pan_images = [f for f in os.listdir(directories['PAN']) if (f.endswith('.tiff') and not f.startswith('_.'))]
    ...
    for pan_img in pan_images:
        ...
        inraster = os.path.join(directories['PAN'], pan_img)
        
        file_out = beach + '_' + date+ '_PAN_sub.tiff'
...
def PanSharpening(directories):
    ...
    for (PAN_img, HS_img) in zip(PAN_cropped_img, HS_cropped_img):
        ...
        beach_pan = PAN_img.split('.')[0].split('_')[0]
        beach_hs = HS_img.split('.')[0].split('_')[0]

        try:
            beach_pan == beach_hs
        except:
            print('Check PAN and HS beaches names')

        date_pan = PAN_img.split('.')[0].split('_')[1]
        date_hs = HS_img.split('.')[0].split('_')[1]

        try:
            date_pan == date_hs
        except:
            print('PAN and HS dates are not the same')
        ...
        name_panS = beach_pan + '_' + date_pan + '_' + 'PANS_sub' + '.tiff'
        out_panS = os.path.join(directories['PanSharp_Square_Cropped'], name_panS)
        
        name_panS_rgb = beach_pan + '_' + date_pan + '_' + 'PANS_sub_rgb'+ '.tiff'
    ...
    print('A total of', u, 'panSharpened images have been saved') # (u/tot_img)*100, '%', end='\r')
...
def crop_PanSharp_to_beach(directories):
    ...
    PRIS_tools.beach_crop_shapefile(directories)
    
    # Shapefiles beach crop
    
    beaches_polygons = [f for f in os.listdir(directories['Crop_shp']) if f.endswith('BeachPol.shp')]
    ...
    for beach in beaches_polygons:
        ...
        beach_and_date = beach.split('_')[0] + '_' + beach.split('_')[1]

        print('Creating: ', beach_and_date + '_PANS_BeachArea.tiff')
        ...
        for img in PanSharp_imgs:
            ...
            if  beach_and_date in img:
                ...
                new_name =  beach_and_date + '_PANS_BeachArea' + '.tiff'
...
def GridRaster(dir_img, crs):
    ...
    for r in range(n_rows):
        for c in range(n_cols):
            ...
            Xcoord, Ycoord = PRIS_tools.pixel_to_world(raster, c, r)
...
def Generate_GridRaster(paths):

        beach = paths['PanSharp_Square_Cropped'].split('/')[-3].split('_')[0]

        dir_grids = paths['scene'] + os.sep + beach + os.sep + 'QGIS' + os.sep + 'Img_grid'

        if not os.path.exists(dir_grids):
            os.mkdir(dir_grids)

        # Load all the images to generate the grid of each one

        imgs = [f for f in os.listdir(paths['PanSharp_Square_Cropped']) if f.endswith('tiff') and 'rgb' not in f]
        ...
        for imagename in imgs:
            ...
            GridName = '_'.join(imagename.split('_')[0:2]) + '_grid.json'
            grid.to_file(os.path.join(dir_grids, GridName), driver = 'GeoJSON')
        


def CropToSquare(coordinates_upper_left, square_length, square_height, paths, beach):

    # Search coordinates that are multiples of 6

    column_upper_left = coordinates_upper_left[0]
    row_upper_left = coordinates_upper_left[1]
    date = os.listdir(os.path.join(paths['scene'], 'PAN'))[0].split('_')[3]

    if column_upper_left%6 == 0:
        column_upper_left = column_upper_left
    else:
        div = column_upper_left/6
        dec, ent = math.modf(div)
        column_upper_left = ent*6
        
    if row_upper_left%6 == 0:
        row_upper_left = row_upper_left
    else:
        div = row_upper_left/6
        dec, ent = math.modf(div)
        row_upper_left = ent*6

    if square_length%6 == 0:
    ...
    pix_col_pan = square_length/6
    pix_row_pan = square_height/6

    if (column_upper_left + pix_col_pan)%6 == 0:
        final_col = column_upper_left + pix_col_pan
    else:
        div = ((column_upper_left + pix_col_pan))/6
        dec, ent = math.modf(div)
        final_col = ent*6
        
    if (row_upper_left - pix_row_pan)%6 == 0:
        final_row = row_upper_left - pix_row_pan
    else:
        div = ((row_upper_left - pix_row_pan))/6
        dec, ent = math.modf(div)
        final_row = ent*6

    for i in ['HS', 'PAN']:
        
        imgs = [f for f in os.listdir(paths[i]) if f.endswith('tiff')]
        
        for img in imgs:

            c0, r0, c1, r1 = column_upper_left, row_upper_left, final_col, final_row #4218, 2184, 4374, 2352
            pathFolderOut = os.path.join(paths['scene'], beach, 'PAN', 'Square_Cropped')
            pathFileIn = os.path.join(paths['scene'], 'PAN', img)


            if i == 'HS':

                c0, r0, c1, r1 = c0/6, r0/6, c1/6, r1/6
                pathFolderOut = os.path.join(paths['scene'], beach, 'HS', 'Square_Cropped')
                pathFileIn = os.path.join(paths['scene'], 'HS', img)



            dc, dr = c1 - c0, r0 - r1

            new_name = beach + '_' + date + '_' + i + '_sub.tiff'

            pathFileOutTif = pathFolderOut + os.sep + new_name

            #
            dset = gdal.Open(pathFileIn)
            width, height = dset.RasterXSize, dset.RasterYSize
            nOfBands = dset.RasterCount
            #
            w, h = min(c0+dc, width) - c0, min(r0+dr, height) - r0

        print('... working on {:}'.format(
            pathFileOutTif[pathFileOutTif.rfind(os.sep)+1:-4]))
        #
        gdaltranString = 'gdal_translate -ot Float32 -of GTIFF -srcwin {:}, {:}, {:}, {:} {:} {:}'.format(
            str(c0), str(r0), str(dc), str(dr), pathFileIn, pathFileOutTif)

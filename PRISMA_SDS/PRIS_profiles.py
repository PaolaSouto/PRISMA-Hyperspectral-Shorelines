...
def generate_profiles_baseline(directories, profiles_car):
    ...
    inputs:
        ...
        * basleine : points defining the baseline
    ...
    for pan_img in pan_img_cr:
        ...
        for pt in baseline:
            ...
            X, Y = PRIS_tools_v2.pixel_to_world(img, pt[0], pt[1])
        ...
        # Interpolate the points each 5 meters (pixels size)
        
        
        # This method will work only with horizontal beaches
        
        #x[0] = x[0] - 5 # -5 pixeles
        #x[-1] = x[-1] + 5 # pixels
        
        #x_new = np.arange(x[0], x[-1], 5)
        ...
        geoson = PRIS_tools_v2.coor_to_geojson(x_new, y_new)
        ...
        EpsgCode_img = PRIS_tools_v2.get_img_epsgCode(directories)
...
class BaselineGenerator(object):
    ...
    def _select_img(self, event):
        ...
        name_baseline =  self.scene + '_' + self.img_names[self.i-1].split('_')[1] + '_bas'+ '.txt'
        ...
        for pt in self.pts:
            ...
            X, Y = PRIS_tools_v2.pixel_to_world(self.img, pt[0], pt[1])
...
def DataFrameProfileCoor(profiles_id, profiles_number, gdf,  ds, transform):
    ...
    for idd, num in zip(profiles_id, profiles_number):
        ...
        for idxs in range(len(gdf['points'][num][idd])):
            ...
            lon, lat = PRIS_tools_v2.world_to_pixel(ds.GetGeoTransform(), point.GetX(), point.GetY())
...
def plot_profiles(directories, prof_coor_pan):
    ...
    _, raster_rgb = PRIS_img_v2.numpy_from_tiff(path_img, True)
...
def profiles_pix_coor(directories, plot):
    ...
    # profiles_path = os.path.join(directories['Profiles'],  directories['PAN'].split('/')[-2] + '_pr.shp')
        ...
        epsg = PRIS_tools_v2.get_img_epsgCode(directories)
...
def profiles_analysis(directories, plot_pr_pix, plot_pix_ss):
    ...
    CW = PRIS_img_v2.ObtainCW(directories ,original_img[0])
    ...
    for i in range(len(pansharp_imgs)):
        ...
        for pr_pix in profiles_pix:
            ...
            if pansharp_img.split('_')[1] in pr_pix:
                ...
                raster_panS, raster_panS_rgb = PRIS_img_v2.numpy_from_tiff(path_img , True)

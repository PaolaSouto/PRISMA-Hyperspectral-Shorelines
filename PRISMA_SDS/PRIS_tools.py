...
#    result_types_folders = ['Img_profiles', 'K_means']
#    
...
#    subfolders = ['csv', 'plot']
    result_types_folders = ['Transition_pix_SpectralSignatures'] # 'PR_pix', 'SS_pix']
...
def get_img_epsgCode(directories):
    ...
    for pan_img in pan_images:
        ...
        if boo_c:
            ...
            path_pan_img = os.path.join(directories['PAN'], pan_img)
...
#                self.cid_accept = self.fig.canvas.mpl_connect('button_press_event', self._accept_bb)#comentar
                self.cid_reject = self.fig.canvas.mpl_connect('button_press_event', self._reject_bb)
    ...
    def _accept_bb(self, event):
        ...
        create_cropping_shapefile(self.directories, self.extent_modi) #extent_modi
...
def define_beach(directories):
    ...
    pan_images = [f for f in os.listdir(directories['PAN']) if f.endswith('.tiff')]
    data = []
    for i in range(len(pan_images)):
        data.append(os.path.join(directories['PAN'], pan_images[i]))
...
def beach_crop_shapefile(directories):
    ...
    profiles_names = [f for f in os.listdir(directories['Profiles']) if f.endswith('_profiles.shp')] #and '90_0' in f]
    
    for profile_name in profiles_names:
        
        beach, date = profile_name.split('_')[0], profile_name.split('_')[1]
        ...
        name_polygon = beach + '_' + date + '_BeachPol.shp'
...
def Square_Crop_file(paths, poly):

    beach = paths['PanSharp_Square_Cropped'].split('/')[-3]
    polygon_geom = Polygon(poly)
    crs = PRIS_tools.get_img_epsgCode(paths)
    polygon = gpd.GeoDataFrame(index=[0], crs=crs, geometry=[polygon_geom]) 
    name_polygon = beach + '_SquarePol.shp'
    dir_polygon = os.path.join(paths['Crop_shp'], name_polygon)
    polygon.to_file(filename= dir_polygon, driver="ESRI Shapefile")
...
def GenerateSAETBandsCombination(path0_prisma, path_saet_data, scene, beach, index, sat):
    ...
    arr = PRIS_img.numpy_from_tiff(dir_file, False)
...
def FromSaetShapefile2Txt(path0_prisma,dir_prisma_sds, dir_saet_sds, scene, beach, sat, index):
    ...
    paths = PRIS_tools.generate_structure(dir_store, scene, beach)
    epsg_code = PRIS_tools.get_img_epsgCode(paths)
